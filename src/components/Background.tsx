...
const Background = ({ width = '100vw', height = '100vh', cameraZ = 125, planeSize = 256, speed = 1.2 }) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      // scrollY increases as we scroll down, time moves forward.
      // If we scroll up, scrollY decreases, time moves backward.
      // Multiply by speed factor to control the movement rate.
      targetTime = window.scrollY * 0.005 * speed; 
    };

    const renderLoop = () => {
      // Smoothly interpolate currentTime towards targetTime for a fluid effect
      currentTime += (targetTime - currentTime) * 0.1;
      plane.uniforms.time.value = currentTime;
    ...
    const init = () => {
      ...
      resize();
      handleScroll(); // Set initial time based on scroll position
      renderLoop();
    };
  ...
  return (
    <div ref={containerRef} style={{ position: 'fixed', top: 0, left: 0, width, height, zIndex: -1, pointerEvents: 'none' }}> 
      <canvas
        ref={canvasRef}
        style={{
          position: 'absolute',
